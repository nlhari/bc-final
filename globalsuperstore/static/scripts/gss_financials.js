...
const salesCount = new dc.DataCount('.dc-data-count');
const salesTable = new dc.DataTable('.dc-data-table');
...
d3.csv('static/data/globalsuperstore.csv').then(data => {
    ...
    data.forEach(d => {
        d.dd = dateFormatParser(d.orderdate);
    ...
    const gss = crossfilter(data);
    const all = gss.groupAll();

    // Dimension by year
    // const yearlyCategoryDimension = gss.dimension(d => {d3.timeYear(d.dd).getFullYear() + '-' + d.category});
    // var yearlyCategoryDimension = gss.dimension(d => d3.timeYear(d.dd).getFullYear());
    const yearlyCategoryDimension = gss.dimension(d => d.subcategory);
    // Maintain running tallies by year as filters are applied or removed
    const yearlyCategoryPerformanceGroup = yearlyCategoryDimension.group().reduce(
        /* callback for when data is added to the current filter results */
        (p, v) => {
            ++p.count;
            p.sales += v.sales;
            p.profit += v.profit;
            p.totalSales = Math.round(p.sales / 10000);
            p.totalProfit = Math.round(p.profit / 1000);
            p.percentProfit = p.profit ? (p.profit / p.sales) * 100 : 0;
    ...
    const dateDimension = gss.dimension(d => d.dd);

    const volumebyDateGroup = dateDimension.group().reduceSum(d => d.sales/1000);
    // Dimension by month
    const moveMonths = gss.dimension(d => d.month);
    // moveMonths.filter(d => {return d.category === 'Technology'});
    // Group by total movement within month
    const monthlyMoveGroup = moveMonths.group().reduceSum(d => d.sales/1000);
    // Group by total volume within move, and scale down result
    

    const volumeByMonthGroup = moveMonths.group().reduceSum(d => d.sales/1000);
    ...
    // const gainOrLoss = gss.dimension(d => d.open > d.close ? 'Loss' : 'Gain');
    // // Produce counts records in the dimension
    // const gainOrLossGroup = gainOrLoss.group();

    // Determine a histogram of percent changes
    const fluctuation = gss.dimension(d => Math.round((d.profit) / d.sales * 100));
    const fluctuationGroup = fluctuation.group();

    // Summarize volume by quarter
    const quarter = gss.dimension(d => {
    ...
    const quarterGroup = quarter.group().reduceSum(d => d.sales);

    // Counts per weekday
    const dayOfWeek = gss.dimension(d => {
    ...
    yearlyBubbleChart /* dc.bubbleChart('#yearly-bubble-chart', 'chartGroup') */
        ...
        .dimension(yearlyCategoryDimension)
        //The bubble chart expects the groups are reduced to multiple values which are used
        //to generate x, y, and radius for each key (bubble) in the group
        .group(yearlyCategoryPerformanceGroup)
    ...
    //Accessor functions are applied to each value returned by the grouping
        ...
        .colorAccessor(d => d.value.totalProfit)
        // `.keyAccessor` - the `X` value will be passed to the `.x()` scale to determine pixel location
        .keyAccessor(p => p.value.totalProfit)
        // `.valueAccessor` - the `Y` value will be passed to the `.y()` scale to determine pixel location
        .valueAccessor(p => p.value.percentProfit)
        // `.radiusValueAccessor` - the value will be passed to the `.r()` scale to determine radius size;
        //   by default this maps linearly to [0,100]
        .radiusValueAccessor(p => p.value.totalSales)
        .maxBubbleRelativeSize(.4)
        .x(d3.scaleLinear().domain([-250, 2500]))
        .y(d3.scaleLinear().domain([-40,50]))
        .r(d3.scaleLinear().domain([0, 700]))
        //##### Elastic Scaling
        ...
        .title(p => [
            p.key,
            `Sales: ${currency(p.value.sales, { formatWithSymbol: true, precision: 0  }).format()}`,
            `Profit: ${currency(p.value.profit, { formatWithSymbol: true, precision: 0  }).format()}`,
            `Profit %: ${(p.value.percentProfit.toFixed(1))}%`,
    ...
    quarterChart /* dc.pieChart('#quarter-chart', 'chartGroup') */
        ...
        .title(d => [
            d.key,
            // `Profit: ${Math.round(p.value.profit)}`,
            `Sales: ${currency(d.value, { formatWithSymbol: true, precision: 0 }).format()}`
            // `Profit in Percentage: ${(p.value.percentProfit.toFixed(1))}%`,
            // `Fluctuation / Index Ratio: ${numberFormat(p.value.fluctuationPercentage)}%`
        ].join('\n'))        
    ...
    fluctuationChart /* dc.barChart('#volume-month-chart', 'chartGroup') */
        ...
        .x(d3.scaleLinear().domain([-100, 52]))
        .xAxisLabel('Profit %')
        .renderHorizontalGridLines(true)
    ...
    //##### Legend
        ...
        .group(monthlyMoveGroup, 'Monthly Sales')
        .valueAccessor(d => d.value)
        // Stack additional layers with `.stack`. The first paramenter is a new group.
        // The second parameter is the series name. The third is a value accessor.
        // .stack(techDimensionGroup, 'Technology Sales')
        ...
        .title(d => {
            ...
            return `${dateFormat(d.key)}\n${currency(value, { formatWithSymbol: true }).format()}`;
        });
    ...
    volumeChart.width(990) /* dc.barChart('#monthly-volume-chart', 'chartGroup'); */
        .height(70)
        .margins({top: 0, right: 50, bottom: 30, left: 30})
    ...
    salesCount /* dc.dataCount('.dc-data-count', 'chartGroup'); */
        .crossfilter(gss)
        .groupAll(all)
    ...
    salesTable /* dc.dataTable('.dc-data-table', 'chartGroup') */
        .dimension(dateDimension)
        ...
        .columns([
            ...
            'profit',
            {
                ...
                label: 'Profit %',
                format: function (d) {
                    var profitpercent = (d.profit / d.sales) * 100;
